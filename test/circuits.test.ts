import {
  ...
  SourceInput,
  VaultInput,
} from "../package/src/prover/hydra-s2-prover";
...
describe("Hydra S2 Circuits", () => {
  ...
  before(async () => {
    ...
    circuitTester = await wasm(
      path.join(__dirname, "../circuits", "hydra-s2.circom")
    );
    ...
    prover = new HydraS2Prover(await commitmentMapperTester.getPubKey());
  ...
  describe("Generating proof of a statement in the vault thanks to HydraS2Account Source", async () => {
  ...
  describe("Verifying accountsTree and registryTree are good", async () => {
    it("Should throw when using an Accounts tree with wrong height", async () => {
      ...
      const prover2 = new HydraS2Prover(
