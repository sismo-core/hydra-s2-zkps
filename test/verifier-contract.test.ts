...
import { HydraS2Verifier, HydraS2Verifier__factory } from "../types";
...
describe("Hydra S2 Verifier contract", () => {
  let commitmentMapperTester: CommitmentMapperTester;
  let accounts: HydraS2Account[];
  let requestIdentifier: BigNumber;
  let hydraS2VerifierContract: HydraS2Verifier;
  ...
  before(async () => {
    ...
    const deployed = await hre.deployments.deploy("HydraS2Verifier", {
      contract: "HydraS2Verifier",
      from: deployer.address,
      args: [],
      skipIfAlreadyDeployed: false,
    });
    hydraS2VerifierContract = HydraS2Verifier__factory.connect(
      deployed.address,
      deployer
    );
  });
  ...
  it("Should be able to generate the proof using the prover package", async () => {
    const prover = new HydraS2Prover(await commitmentMapperTester.getPubKey());
  ...
  it("Should be able to verify the proof using the verifier", async () => {
    const isValidContract = await hydraS2VerifierContract.verifyProof(
      proof.a,
      proof.b,
      proof.c,
      proof.input
